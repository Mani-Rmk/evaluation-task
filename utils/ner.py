...
# Imporved Price matcher can handle both numeric and word representations of prices
number_words = [
...
price_patterns = [
    ...
    [{"LOWER": {"IN": number_words}}, {"LOWER": {"IN": number_words}}],
]
price_matcher.add("PRICE", price_patterns)
...
def ner_data(USER_INPUT: str):
    ...
    #     str_prices.append(first_token)
    #     try:
    #         if span[0].like_num:
    #             number_value = float(span[0].text)
    #         else:
    #             number_value = w2n.word_to_num(str_prices[0])
    #         prices.append(number_value)
    #     except ValueError:
    #         continue
    
    # Using the improved price matcher
    ...
    for span in spans:
        ...
        #print("Matched span:", text) -debugging line
        try:
            if text.replace('.', '', 1).isdigit():
                number_value = float(text)
            else:
                number_value = w2n.word_to_num(text)
            prices.append(number_value)
        except ValueError:
            continue
    ...
    #print("nerprice:",prices) - debugging line
    return {
        "place": places if places else {},
        "country": country if country else {},
        "price": sum(prices) if prices else None,
        "amenities": amenities,
